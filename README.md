...
| NUTS3 geometries | **Real** — all 1345 EU NUTS3 regions, Eurostat GISCO 2024 boundaries (`data/NUTS_RG_10M_2024_4326.gpkg`, EPSG:4326), read via `pipeline/gpkg_reader.py` (no GDAL/geopandas dependency). The 14 outermost-region NUTS3 codes (Canarias/Açores/Madeira/French RUP) use higher-resolution 1M geometry instead (`data/outermost_regions_01M_4326.geojson`), since they're rendered zoomed in tight on `docs/index.html`'s outermost-region insets |
